...
func TestNilAndEmptyValues(t *testing.T) {
	...
	t.Run("empty key operations rejected", func(t *testing.T) {
...
func TestBatchOperationsEdgeCases(t *testing.T) {
	...
	t.Run("SetBatch with partial failures simulation", func(t *testing.T) {
		items := []planar.KVItem{
			...
			{Key: []byte(""), Value: []byte("invalid key")},
			{Key: []byte("normal2"), Value: []byte("value2")},
		}

		_, err := db.SetBatch(items)
		if err == nil {
			t.Error("expected error for batch with empty key")
		}
	})
}
...
func TestMapBackendMultipleStores(t *testing.T) {
	logger := slog.Default()

	store1Backend := OpenMapBackend("store1")
	defer store1Backend.Close()
	store1, err := store1Backend.LoadOrCreate(logger, "store1")
	if err != nil {
		t.Fatalf("failed to create store1: %v", err)
	}

	store2Backend := OpenMapBackend("store2")
	defer store2Backend.Close()
	store2, err := store2Backend.LoadOrCreate(logger, "store2")
	if err != nil {
		t.Fatalf("failed to create store2: %v", err)
	}

	store1.Set([]byte("key"), []byte("value1"))
	store2.Set([]byte("key"), []byte("value2"))

	val1, _ := store1.Get([]byte("key"))
	val2, _ := store2.Get([]byte("key"))

	if !bytes.Equal(val1, []byte("value1")) {
		t.Errorf("store1 value mismatch: got %q, want %q", val1, "value1")
	}
	if !bytes.Equal(val2, []byte("value2")) {
		t.Errorf("store2 value mismatch: got %q, want %q", val2, "value2")
	}

	sameStore1, _ := store1Backend.LoadOrCreate(logger, "store1")
	val3, _ := sameStore1.Get([]byte("key"))
	if !bytes.Equal(val3, []byte("value1")) {
		t.Error("should return existing store with same data")
	}

	defaultStoreBackend := OpenMapBackend("")
	defer defaultStoreBackend.Close()
	defaultStore, _ := defaultStoreBackend.LoadOrCreate(logger, "default")
	_, err = defaultStore.Get([]byte("key"))
	if err == nil {
		t.Error("default store should not have key from named stores")
	}
}

func setupTestDB(t *testing.T) planar.KV {
	db, err := newMapKV()
	if err != nil {
